defmodule StateServer do
  ...
  (atoms) as keys, and destination states as values.
  ...
  **transition** and **update** events are special.  If they are at the head of the event
  ...
  @doc false
  @spec states_to_typelist([atom]) :: Macro.t
  def states_to_typelist([state]), do: state
  def states_to_typelist([state1, state2]), do: {:|, [], [state1, state2]}
  def states_to_typelist([state | rest]), do: {:|, [], [state, states_to_typelist(rest)]}
  ...
  defmacro __before_compile__(env) do
    ...
    unless StateGraph.valid?(state_graph) do
      raise %CompileError{file: env.file, line: 1, description: "@state_graph is malformed"}
    end
    ...
    quote do
      ...
      @__terminal_states__ StateGraph.terminal_states(@state_graph)
      @__terminal_transitions__ StateGraph.terminal_transitions(@state_graph)
      ...
      #@impl true
      #defguard is_terminal(state) when state in @__terminal_states__

      #@impl true
      #defguard is_terminal(state, transition) when {state, transition} in @__terminal_transitions__
    end
  end
