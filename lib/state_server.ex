defmodule StateServer do
  ...
  **NB** a continuation is simply an `:internal` event with a reserved word
  ...
  @doc """
  triggered just prior to entering a state; the entry into the new state cannot
  be cancelled.  If entering a state is done with a `:goto` statement or a
  `:gen_statem` state change, `transition` will be nil.
  ...
  - `:timeout`
  """
  @callback on_state_entry(state :: atom, transition :: atom, data :: term) ::
    :noreply | on_state_entry_event
  ...
  @optional_callbacks [handle_call: 4, handle_cast: 3, handle_info: 3,
    ...
    on_state_entry: 3, is_terminal: 1, is_terminal_transition: 2, is_edge: 3]
  ...
  defp make_function_for(module, fun, state)
       when fun in [:handle_cast, :handle_info, :handle_internal, :handle_continue, :handle_timeout] do
    shim_fn_name = state_shim_for(fun)
    quote do
      @doc false
  ...
  defmacro defer({fun, _, _}) when fun in
      [:handle_cast, :handle_info, :handle_internal, :handle_continue, :handle_timeout, :handle_transition] do
    quote do
