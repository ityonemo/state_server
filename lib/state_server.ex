defmodule StateServer do
  ...
  **Defining the state graph is required**.
  ...
  After definition, the state graph will define several utility functions that
  `StateServer` uses to marshal your state machine through the state graph
  correctly.  Furthermore, you are given the following guards: `c:is_terminal/1` and
  `c:is_terminal_transition/2` that will help you, and the `state` and `transition`
  types, which are unions of atom literals, are also defined.
  ...
  **Transitions** represent the main business logic of your state machine.  They come
  ...
  """
  @macrocallback is_terminal_transition(state::any, transition::any) :: Macro.t
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      @impl true
      defguard is_terminal_transition(state, transition) when {state, transition} in unquote(terminal_transitions)
